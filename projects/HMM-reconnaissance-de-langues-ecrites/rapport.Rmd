...
On importe les textes à partir d'un fichier csv.                                                                                   
...
Dans cette partie, On effectue les actions suivantes sur les textes:   
...
On crée une `matrice f` qui contient la fréquence des symboles dans un texte.                                                                          
...
cat("\n")
print("La matrice de fréquence pour les 10 premiers textes est: ")
print(f[1:10,1:27])
...
On  utilise la fonction `log` sur les fréquences telles que Xij = log(1+fij) pour harmonizer les données 
...
Dans cette partie, on affiche les log fréquences des textes en français en anglais et en français
...
print("Les log  fréquences en français: ")
barplot(logFreqFrench$tabNormalizedWithLabels)
# barplot(test1$tabNormalizedWithLabels)
cat("\n")

logFreqEnglish = showHist(X, y, 1)
print("Les log fréquences en anglais: ")
barplot(logFreqEnglish$tabNormalizedWithLabels)
...
### Commentaires
  On peut remarquer pour les textes en français, on a plus grande fréquence des lettres "e", "j" et "q" qu'en anglais.
Parallèlement, on observe une plus grande fréquence des lettres "k", "y" et "w" qui sont très présence dans la langue anglaise.
Ces fréquences sont très logique compte tenu de ces deux langues et de la structure de leurs mots respectifs.


# Exercice 2: 
On va utiliser **l'algorithme EM** pour déterminer les paramètres de la mixture gaussienne
...
## Fonction etape E
Ici on calcule la vraisemblance
...
## Fonction etape M
Dans cette partie, on cherche les paramètres qui maximisent la vraisemblance
...
On effectue les trois étapes jusqu'à convergence.
Notre test se fait sur le logLikehood, on regarde la différence du logLike du précédent calcul et l'actuel par rapport à tol = 10^-6
...
Cette partie est en deux étapes, on évalue la performance du classifieur:                                           
...
# On transforme les 1 en -1 et les 2 en 1 de nos prédictions
...
# On prédit la classe en prenant le max entre les 2 colonnes pour chaque ligne
getClass = function(E) {
...
Grâce à la fonction `createDataPartition` de `caret`, on effectue une validation croisée sur nos données.
On procède comme suit:      

* On fait les données d'entraînement et de test de façon aléatoire avec `createDataPartition` de la librairie `caret`                                      
* On évalue les paramètres sur les 30 derniers textes et on evalue ses performances sur les 30 premiers textes
* On réalise un kfold sur 3 groupes de textes avec les paramètres de 60 textes comme données d'entraînement et 30 textes comme test 
...
Dans cette partie, on a les paramètres sur les 30 premiers textes et on aimerait évaluer ses performances sur les 30 restants
...
Ici on réalise un kfold sur nos données.
...
Dans cette partie on crée un classifieur markovien basé sur les matrices de transitions entre nos différents symboles des différentes.
...
normaliseMatrix = function(mat) {
  ...
  ########### On additionne à notre matrice initiale pour éviter le problème de division par zéros
...
# La classe pour laquelle on estime les paramètres y = -1 ou  y = 1

estimMarkovParameters = function(lines, y, type) {
  
  # On détermine dans y les indcices où y = type
  indices = which(y == type)
  
  # On récupère les textes correspondants aux indices
  texte = lines[indices]
  ...
  # On normalise la matrice en ajoutant 1 à chaque élément(pour éviter la division par zéro) de la ligne et 
  # on divise chaque élément par la somme des éléments
...
On simule d'abord les deux chaînes de markov à partir des paramètres calculés précédemment.

A partir de notre probabilité d'émission une matrice de dimension 30x2, on prédit la classe des textes.
...
# On a deux chaînes de markov simulées suivant les paramètres A1, Pi1, A2, Pi2
...
# On transforme les 1 en -1 et les 2 en 1
...
**On remarque que dans cette méthode, à chaque étape la probabilité d'émission du caractère et du caractère précédent joue un rôle important dans la détection des passages et des couples de caractères(bigrammes)**

# Exercice4: Baum-Welch

## 1. Algorithme de Baum-Welch pour deux états cachés

Dans cette partie, on code les fonctions `forward`, `backward` ainsi que les méthodes d'estimation de la `matrice de transition`` et des probabilités d'émission` pour deux états cachés.                                                                                       
Le plus gros problème lors de la modélisation d'un HMM étant de l'imprécision des flottants, nous utiliserons dans les fonctions `forward`, `backward` des méthodes pour éviter de tendre vers l'infini ou être à zéro.
...
Ici on estime ou réestime la `matrice de transion`  et les `probabilités d'émission`
...
Dans cette partie, on excécute **l'algorithme de Baum-Welch** en utilisant comme initialisation les informations de l'exercice précédent.                                              
...
Dans cette partie, on teste les paramètres obtenus par **l'algorithme de Baum-Welch**, on fait appel à **l'algorithme de Viterbi** avec comme A et B les paramètres données par **l'algorithme de Baum-Welch**.
...
**On remarque qu'avec une idée de l'initialisation des probabilités du HMM que nous recherchons  même approximative, l'algorithme de baum-welch converge et on se rapproche des vraies valeurs de A et de B**


## 4. Baum-Welch avec initialisation aléatoire

Dans cette partie, on teste les paramètres estimés par **l'algorithme de Baum-Welch** sur des paramètres initialisés aléatoirement.                                               
...
Avec une initialisation aléatoire, on a pas les bons paramètres correspondant à notre séquences

## 5. Commentaires

**On voit bien que l'algorithme de baum-welch présente cependant le défaut de ne pas toujours converger vers la même solution, voire même de ne pas converger du tout sous certaines conditions.**                                 
...
## On affiche un graphique d'adjacence entre nos textes à partir de K

Dans cette partie, on affiche un graphique à partir de K.
Vous pourrez vérifier que ça marche, on affiche à côté les indices des textes par langues dans notre base de texte
...
plot(graph_from_adjacency_matrix(K,mode="undirected"),vertex.color=tmpY)
...
**On a parfois une interversion des paramètres, probablement dû à l'initialisation des paramètres du VEM. Il y a le même problème dans votre exemple lors de la dernière séance**
...
Différences parmi les classifieurs étudiés :   
 ...
 - Baum-Welch marche mieux quand on initialise avec les bons paramètres (ici nos 2 matrices de transition CF exercice3), tandis que si on l'initialise         aleatoirement, on a parfois peu ou pas de convergence vers une solution correspondante à nos observations                              
 - VEM basé sur la matrice de similarité, si des mots ont des frequences qui se rapprochent mais sont de deux langues differentes, on peut avoir le risque d'une mauvaise classification                                                                              

Avec une base de textes bien fournis et des textes mieux structurés, on peut affirmer que le classifieur de bayes et le VEM sont les plus performants.       
